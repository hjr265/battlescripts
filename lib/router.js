...
.post(function(req, res, next) {
	async.auto({
		myBot: function(done) {
			Bot.findById(req.body.bot, done)
		},
			...
			switch(true) {
				case !!req.body.opponent.match(/^.+@.+$/):
					User.findByEmail(req.body.opponent, function(err, user) {
						...
						if(!user) {
							req.flash('info', 'Invalid opponent')
							res.redirect('back')
							return done(true)
						}
				...
				case !!req.body.opponent.match(/^@.+$/):
					User.findByTwitterHandle(req.body.opponent, function(err, user) {
						if(err) {
							return next(err)
						}
						if(!user) {
		...
		challenge: ['myBot', 'other', function(done, ctx) {

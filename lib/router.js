...
.post(function(req, res, next) {
	if(req.limit.count > process.env.OPT_MAX_SAVES_PER_HOUR) {
		req.flash('info', 'You may save bots at most '+process.env.OPT_MAX_SAVES_PER_HOUR+' times every hour')
		return res.redirect('back')
	}

	switch(true) {
		case !validator.isLength(req.body.name, 1, 32):
			req.flash('info', 'Name should be between 1 and 32 characters long')
			return res.redirect('back')
	}
	...
	if(issue) {
		req.flash('info', 'Code validation failed: '+issue.error)
		return res.redirect('back')
	}
...
.all(function(req, res, next) {
	...
	.exec(function(err, count) {
		...
		if(count > 0) {
			req.flash('info', 'You cannot modify a bot enrolled in a contest')
			return res.redirect('back')
		}
...
.post(function(req, res, next) {
	...
	.exec(function(err, count) {
		...
		if(count > 0) {
			req.flash('info', 'You cannot remove a bot enrolled in a contest')
			return res.redirect('back')
		}
...
.post(function(req, res, next) {
	if(req.limit.count > process.env.OPT_MAX_TESTS_PER_HOUR) {
		req.flash('info', 'You may run at most '+process.env.OPT_MAX_TESTS_PER_HOUR+' test battles every hour')
		return res.redirect('back')
	}

	var trackId = req.user.id.toString() + '-' + moment().unix() + '-' + Math.random()*1e9
	...
	function emit(data) {
		data.no = ++emit.no
		req.io.to('test-battle:'+trackId).emit('test-battle:step', data)
	}
	emit.no = 0
		...
		fn: function(done) {
			...
			async.auto({
				...
				battle: ['bots', 'engine', function(done, ctx) {
					...
					battle.rel.trackId = trackId
					battle.expiry = moment().add(2, 'hour').toDate()
...
var testQ = async.queue(function(task, done) {
	task.fn(done)
}, process.env.OPT_MAX_TESTS_CONCURRENCY)

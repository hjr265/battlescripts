...
.get(function() {
	if(this.profiles.twitter) {
		return '@'+this.profiles.twitter.username
	} else {
		return this.email
	}
})
...
schema.statics.findByTwitterUsername = function(username, done) {
	return User.findOne({
		'profiles.twitter.username': username.replace(/^@/, '')
	}, done)
}
...
schema.statics.findByHandle = function(handle, done) {
	return User.findOne({
		$or: [
			{
				email: cleanEmail(handle)
			},
			{
				'profiles.twitter.username': handle.replace(/^@/, '')
			}
		]
	}, done)
}

...
schema.statics.createFromTwitter = function(profile) {
	...
	user.profiles.twitter.handle = JSON.parse(user.profiles.twitter._raw).screen_name
	return user
}
...
schema.statics.findByTwitterHandle = function(handle, done) {
	return User.findOne({
		'profiles.twitter.handle': handle.replace(/^@/, '')
	}, done)
}
